...
- (void)mostrarMensagem:(NSString *)titulo msg:(NSString *)mensagem {
    UIAlertView *alerta = [[UIAlertView alloc]
                           initWithTitle:titulo message:mensagem delegate:self cancelButtonTitle:@"OK" otherButtonTitles: nil];
    [alerta show];
    alerta = nil;
}
...
-(IBAction)compartilhar:(id)sender{
    UIActionSheet *popup = [[UIActionSheet alloc]
                            initWithTitle:@"compartilhar" delegate:self
                            cancelButtonTitle:@"Voltar" destructiveButtonTitle:nil
...
-(void)compartilharEmail {
    if ([MFMailComposeViewController canSendMail]) {
        ...
        [email setToRecipients:[NSArray arrayWithObjects:@"dyego.donadone@gmail.com", @"dyegodonadone@hotmail.com", nil]];
...
-(void)listarDados{
    for(NSDictionary *item in dados){
        NSLog(@"Alcool: %@. Gasolina: %@", [item objectForKey:@"Alcool"], [item objectForKey:@"Gasolina"]);
    }
    NSLog(@"************************************************************");
}
